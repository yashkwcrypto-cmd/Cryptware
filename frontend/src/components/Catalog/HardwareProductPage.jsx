...
import { fetchProducts as fetchProductsApi } from '../../util/Requests';
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <button
                ...
                className={`flex flex-col items-center justify-center gap-1.5 py-2.5 rounded-xl border-2 text-[0.7rem] font-bold transition-all duration-200 ${sendVia === 'gmail'
                  ? 'border-[#06a3da] bg-[#06a3da]/10 text-[#06a3da] shadow-sm'
                  : 'border-gray-200 bg-[#f8faff] text-[#64748b] hover:border-[#06a3da]/40'
                  }`}
              >
...
// ─── Image Zoom ───────────────────────────────────────────────────────────────
function ImageZoom({ src, alt, className, onError }) {
  ...
  return (
    <div 
      className={`relative cursor-zoom-in overflow-hidden flex items-center justify-center z-10 ${className}`}
    ...
    >
      <img 
        src={src} 
        alt={alt} 
        className="w-full h-full object-contain drop-shadow-lg transition-transform duration-200 ease-out" 
        style={zoom.active ? {
...
// ─── Detail Panel ─────────────────────────────────────────────────────────────
function DetailPanel({ item, onClose, onInquiry, onLabelQuote, products = [] }) {
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="h-[280px] relative bg-gradient-to-br from-[#f8faff] to-white flex items-center justify-center border-b border-gray-100 overflow-hidden">
            ...
            {!imgError ? (
              <ImageZoom src={item.img} alt={item.title} className="max-h-[220px] max-w-[80%]" onError={() => setImgError(true)} />
            ) : (
        ...
        <div className="px-6 py-4 border-t border-gray-100 bg-white flex gap-3">
          ...
          <button
            onClick={() => isLabel && onLabelQuote ? onLabelQuote(item) : onInquiry(item)}
...
function ProductCard({ item, onOpen, onLabelQuote, viewMode = 'grid' }) {
  ...
  const renderQuoteBtn = (extraClass = '') => (
    <button
...
export default function HardwareProductPage() {
  ...
  return (
    <div className="relative min-h-screen bg-[#f8faff] font-sans text-[#1a1a2e] overflow-x-hidden">
      ...
      {/* ── DETAIL PANEL ─────────────────────────────────────────────────────── */}
      {selectedItem && (
        <DetailPanel item={selectedItem} onClose={() => setSelectedItem(null)} onInquiry={handleInquiry} onLabelQuote={setLabelQuoteItem} products={products} />
      )}
